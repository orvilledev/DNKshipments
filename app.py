...
if uploaded is None:
    ...
    with st.expander("What the app produces"):
        st.markdown(
            ...
            "present (often around column O or P). If a later carton drops the "
            "UPC label but keeps the values in the same column, that column is "
            "still used. If the export has no UPC header at all, the barcode is "
            "inferred from the unlabeled columns between Size and Description. "
            "Older exports without barcodes fall back to the `Item` number. "
            "Values are kept exactly as printed (including any leading zeros).\n"
